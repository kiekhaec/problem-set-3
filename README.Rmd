...
```{r problem_1}
# first gather data and drop gene name then break key column out with separate
...
cleaned_data %>%
    ...
    time.point = str_c('hpi.', time.point)) -> expr_data
    
expr_data %>% spread(time.point, value) -> expr_data
...
```{r problem_3}
expr_data %>% 
  mutate(expr.diff = hpi.24 - hpi.6) -> expr_data

expr_data %>% 
  group_by(plant, fungus, gene.name) %>%
  summarise(mean.expr.diff = mean(abs(expr.diff)), expr.diff.var = var(expr.diff)) %>% 
  arrange(desc(mean.expr.diff), expr.diff.var) %>%
  group_by(plant, fungus) %>% 
  slice(1:3) -> top3_expr_data
...
The process of tidying the data was initiated by generating a tibble using  as_data_frame(raw_data) and it has `r nrow(raw_data)` rows. The resultant tibble contained `r nrow(cleaned_data)` rows and `r ncol(cleaned_data)` columns. For determination of expression analysis the data was first gathered and then separated to generate new `r ncol(cleaned_data)` columns for each time point from hpi.6 to hpi.24. This enabled generation of an additional, `r ncol(expr_data)` column, enabling calculation of the differential expression between the highest and lowest time points for each of the `r nrow(cleaned_data)` 2rows

## Problem 5

Plot the expression (`value`) by time (`hpi`) for the above genes. Format the plot as follows:
Per Kent Riemondy: Plot the raw value per time for each gene. Filter the original data to extract out the gene names that you find in problem 3, then plot the expression values for these genes.

1. Plot each point. Add: + geom_point() 
1. Connect the points with a smooth line Add: + geom_smooth(method = 'lm', se = FALSE) 
1. Create a facet for each `plant` / `bgh` pair Add: + facet_wrap(`plant` ~ `bgh`) 
1. Assign a different color to each gene Add: color = factor(gene.name) 
1. Assign a different shape to each replicate Add: aes(x=time.point, y=value, shape = rep)
1. Add a descriptive title to the plot Add: ggtitle('Differentially Expressed Genes in Immunocompromised Plants') 
1. Use the "black & white" theme Add:  + theme_bw()
...
filter(cleaned_data, gene.name %in% top3_expr_data$gene.name) -> cleaned_data_by_genes

cleaned_data_by_genes %>% 
  mutate(time = str_replace(time.point, "hpi", ""),
         time = as.integer(time)) -> cleaned_data_by_genes

ggplot(cleaned_data_by_genes, 
       aes(x=time, 
           y=value, 
           size = rep, 
           color = factor(gene.name))) +  
  geom_point() + 
  geom_smooth(method = 'lm', se = FALSE, size = 1) +
  facet_grid(plant ~ fungus) + 
  theme_bw() +
  ggtitle("Arabidopsis Fungal mRNA Expression")
